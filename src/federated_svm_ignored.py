...
def run_federated_svm(X_train, y_train, X_test, y_test, num_clients, num_rounds):
    ...
    for _ in range(num_rounds):
        ...
        for idx in client_indices:
            ...
            if len(np.unique(y_c)) < 2:
                continue
            X_c = X_train[idx]
            clf_local = SGDClassifier(loss='hinge', max_iter=1, tol=None)
    ...
    clf_global = SGDClassifier(loss='hinge')
...
def run_federated_svm_noniid(X_train, y_train, X_test, y_test, num_clients, num_rounds):
    ...
    for i in range(num_clients):
        if i % 2 == 0:
            size_0 = int(len(class_0_idx) / num_clients)
            size_1 = int(size_0 * 0.2)
        else:
            size_1 = int(len(class_1_idx) / num_clients)
